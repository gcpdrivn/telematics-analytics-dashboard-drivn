...
filled in from the template seed_dimensions.py generates.
...
- seed_dimensions.py reads it (via read_and_clean_vehicle_master) as the
  ...
  on every reseed, falling back to telemetry-derived values only for plates
...
def write_cleaned_copy(path: Path, cleaned_df: pd.DataFrame) -> None:
    ...
    display_df = cleaned_df.rename(columns={v: k for k, v in _COLUMN_RENAMES.items()})
    display_df = display_df[list(_COLUMN_RENAMES.keys())]
